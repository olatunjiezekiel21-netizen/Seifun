...
diff --git a/netlify/functions/wallet-interactions.ts b/netlify/functions/wallet-interactions.ts
--- a/netlify/functions/wallet-interactions.ts
+++ b/netlify/functions/wallet-interactions.ts
@@ -0,0 +1,42 @@
...
+  try {
+    const { address, network = 'mainnet', lookbackBlocks = 200_000 } = JSON.parse(event.body || '{}') as { address?: string; network?: 'mainnet' | 'testnet'; lookbackBlocks?: number }
...
+
+    // Native transfers (scan recent txs)
+    const nativeTransfers: Array<{ to: string; from: string; value: string; txHash: string; blockNumber: number }> = []
+    // Note: Without an indexer this requires scanning tx history via external APIs; we keep this minimal.
+
