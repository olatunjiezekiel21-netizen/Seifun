...
export const handler: Handler = async (event) => {
	...
	try {
		const body = JSON.parse(event.body || '{}') as {
			userId?: string; wallet?: string; advice: string; caution?: string;
			confidence_score?: number; recommendation?: string; context?: any;
		}
		const client = new MongoClient(MONGODB_URI)
		await client.connect()
		const col = client.db(MONGODB_DB).collection(COLLECTION)
		const doc = {
			userId: body.userId || null, wallet: body.wallet || null, advice: body.advice,
			caution: body.caution || null,
			confidence_score: typeof body.confidence_score === 'number' ? body.confidence_score : null,
			recommendation: body.recommendation || null, context: body.context || {},
